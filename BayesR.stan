	data{ 
	int<lower=0> Px;
	int<lower=0> N;
	matrix[N,Px] X;
	real y[N];
}
parameters{
vector[Px] beta; // flat prior 

// this gives the lower bound for the variance components
real<lower=0> sigma;
// this is our vector of marker-specific variances
real<lower=0> tau;
vector[N] MU;
simplex[4] pi[Px];
}

transformed parameters{
real lp;
{
  vector[4] beta1; // flat prior 
  real accum;
  accum=0;
  for(i in 1:Px){ //mixture contributions to the joint distribution
    beta1[1]=log(pi[i,1])+normal_lpdf(beta[i]|0,1e-6*tau);
    beta1[2]= log(pi[i,2])+normal_lpdf(beta[i]|0,1e-3*tau);
    beta1[3]=log(pi[i,3])+normal_lpdf(beta[i]|0,1e-2*tau);
    beta1[4]=log(pi[i,4])+normal_lpdf(beta[i]|0,1e-1*tau);
    accum= accum+log_sum_exp(beta1);
  }
  lp=accum;
}
}
model{
tau ~ normal(0,1); //normal prior on variances as recommended in stan page
sigma ~ normal(0,1); // normal prior on variance as recommended in stan page
MU ~ cauchy(0,1);   // fat tailed prior on the means
// the likelihood (vector expression)
y ~ normal(MU + X * beta, sigma);

target += lp; //mixture contribution to the joint distribution
}

generated quantities{
  vector[N] ypred;
  for(i in 1:N)
    ypred[i] = normal_rng(MU[i] +X[i,]*beta,sigma);
}
